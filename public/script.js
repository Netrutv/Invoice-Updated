...
function loadSavedLogo() {
    logoBase64 = localStorage.getItem('quickbill_logo') || "";
    renderLogo();
}
...
function renderLogo() {
    ...
    if (!preview) return;
    preview.innerHTML = logoBase64
        ? `<img src="${logoBase64}" alt="Business Logo">`
        : "";
}
            ...
            tr.innerHTML = `
                <td>
                    <div class="item-text-stack">
                        <input type="text" value="${productName}" placeholder="Product Name" class="item-name">
                        <textarea placeholder="Description" class="item-description">${productDescription}</textarea>
                    </div>
                </td>
...
function addNewRow() {
    ...
    tr.innerHTML = `
        <td>
            <div class="item-text-stack">
                <input type="text" placeholder="Product Name" class="item-name">
...
async function saveToDB() {
    ...
    const invoiceData = {
        ...
        items: Array.from(document.querySelectorAll('#rows tr')).map(row => {
            ...
            const productDescription = row.querySelector('.item-description').value.trim();

            return {
                name: productName,
                details: productDescription,
                description: [productName, productDescription].filter(Boolean).join('\n'),
...
async function loadSettings() {
    const savedProfile = JSON.parse(localStorage.getItem('quickbill_profile') || '{}');
    try {
        const res = await fetch(`${API_URL}/profile`);
        const data = await res.json();
        const profile = data.name ? data : savedProfile;
...
function applyProfile(profile) {
    ...
    const upi = DEFAULT_UPI_ID;

    document.getElementById('set-biz-name').value = safeProfile.name || "";
    document.getElementById('set-biz-addr').value = safeProfile.address || "";
    document.getElementById('set-biz-gst').value = safeProfile.gstin || "";
    document.getElementById('set-biz-upi').value = upi;
    document.getElementById('biz-name').innerText = safeProfile.name || "BUSINESS NAME";
    document.getElementById('biz-addr').innerText = safeProfile.address || "Your Address Line 1, City, PIN";
    document.getElementById('biz-gst').innerText = safeProfile.gstin || "27AAAAA0000A1Z5";
    window.userUPI = upi;
}
...
function downloadPDF() {
    ...
    style.innerHTML = `
        ...
        .table-wrapper input,
        .table-wrapper textarea { border: none !important; background: transparent !important; padding: 0 !important; box-shadow: none !important; resize: none !important; }
